#!/usr/bin/env python3
"""Audit entire project for models missing descriptions.
...
def main() -> int:
    ...
    for sql_file in models_dir.rglob('*.sql'):
        ...
        if not model_has_description(model_name, sql_file, yaml_files):
            missing_descriptions.append(str(sql_file))

    print(f"Scanned {files_checked} model files\n")

    if missing_descriptions:
        print(f"FOUND {len(missing_descriptions)} models missing descriptions:\n")
