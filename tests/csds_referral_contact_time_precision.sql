...
with event_times as (
    select referral_received_date as event_date, referral_received_time as event_time,
        referral_received_at as event_at, referral_received_time_precision as precision
    from {{ ref('fct_csds_referral') }}
    union all
    select care_contact_date, care_contact_time, care_contact_at, care_contact_time_precision
