...
function New-DbtEnvFile {
    ...
    $account = Read-Host "Snowflake account identifier (e.g. ATKJNCU-NCL)"
    $user = Read-Host "Snowflake username (e.g. your.name@nhs.net)"
    $role = Read-Host "Snowflake role [DBT_ADMIN]"
    if (-not $role) { $role = 'DBT_ADMIN' }
    $warehouse = Read-Host "Snowflake warehouse [WH_WNL_ENGINEERING_M]"
    if (-not $warehouse) { $warehouse = 'WH_WNL_ENGINEERING_M' }
    ...
    $lines = @(
        "# Generated by start_dbt.ps1 onboarding. Never commit this file.",
