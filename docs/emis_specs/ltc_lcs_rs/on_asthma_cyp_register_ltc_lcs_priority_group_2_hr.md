<!-- Extracted from 'NCL LTC LCS R5 updated 27112025.xml' (sha256 e4984ab973047c074d5cecc3264b29e424751894cf342b1ff371b39c39f5e8f8)
     report id: 0o2bs0c0-hs1r-kw-0g0s-1xatcpz07291
     folder: NCL LTC LCS R5.0 updated: 27112025 > 6) Data Quality > zHouse keeping > zSupporting Searches > Risk Stratification R2 > Disease
     extracted: 2026-06-09 by scripts/extract_emis_ltc_lcs_specs.py
     Readable guide only: for exact operators/ranges query the agent API
     (agentInterpretation.decisionFlow[].criteriaDetails). -->

# On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)*
...
- **Clinical Codes** (clinical events)
  - Code in: `on_asthma_cyp_reg_pg2_hr_vs1` (4 codes), or `on_asthma_cyp_reg_pg2_hr_vs2` (1 code)
  - Keep only the latest matching record
- **Clinical Codes** (clinical events)
  - Code in: `on_asthma_cyp_reg_pg2_hr_vs3` (2 codes), or `on_asthma_cyp_reg_pg2_hr_vs4` (1 code)
  - Keep only the latest matching record
...
- **Clinical Codes** (clinical events)
  - Code in: `on_asthma_cyp_reg_pg2_hr_vs5` (3 codes)
...
A patient matches this rule when:
- **Clinical Codes** (clinical events)
  - Code in: `on_asthma_cyp_reg_pg2_hr_vs6` (1 code)
  - Where date within the last 12 months
  - Where numeric value >= 2
...
A patient matches this rule when:
- **Medication Issues**
  - Code in: `on_asthma_cyp_reg_pg2_hr_vs7` (3 codes)
  - Where drug code in `on_asthma_cyp_reg_pg2_hr_vs7` (3 codes)
...
A patient matches this rule when:
- **Medication Issues**
  - Code in: `on_asthma_cyp_reg_pg2_hr_vs8` (5 codes)
  - Where drug code in `on_asthma_cyp_reg_pg2_hr_vs8` (5 codes)
  - Where issue date within the last 12 months
...
A patient matches this rule when:
- **Medication Issues**
  - Code in: `on_asthma_cyp_reg_pg2_hr_vs9` (3 codes)
  - Where drug code in `on_asthma_cyp_reg_pg2_hr_vs9` (3 codes)
...
A patient matches this rule when:
- **Medication Issues**
  - Code in: `on_asthma_cyp_reg_pg2_hr_vs10` (1 code), or `on_asthma_cyp_reg_pg2_hr_vs11` (28 codes)
  - Where issue date within the last 12 months
  - Where drug code in `on_asthma_cyp_reg_pg2_hr_vs10` (1 code), `on_asthma_cyp_reg_pg2_hr_vs11` (28 codes)
...
- **Medication Issues** — patient must NOT have a matching record
  - Code in: `on_asthma_cyp_reg_pg2_hr_vs12` (6 codes)
  - Where issue date within the last 12 months
  - Where drug code in `on_asthma_cyp_reg_pg2_hr_vs12` (6 codes)
- **Medication Issues**
  - Code in: `on_asthma_cyp_reg_pg2_hr_vs9` (3 codes)
  - Where drug code in `on_asthma_cyp_reg_pg2_hr_vs9` (3 codes)
  - Where issue date within the last 3 months
...
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs1` |  | SNOMED | 4 | Child on protection register, Child removed from protection register, Child n... | 6528bece |
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs10` |  | SCT Const | 1 | Bambuterol Hydrochloride | 42efbe32 |
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs11` |  | SCT_PREP | 28 | Atimos Modulite 12micrograms/dose inhaler (Chiesi Ltd), Foradil 12microgram i... | f9bf039c |
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs12` |  | SCT Const | 6 | Beclometasone Dipropionate, Budesonide, Ciclesonide +3 more | a2d6b25c |
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs2` |  | SNOMED | 1 | Child on protection register | d9a584fb |
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs3` |  | SNOMED | 2 | Child in need, Child no longer in need | 8332ea34 |
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs4` |  | SNOMED | 1 | Child in need | 17e62b70 |
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs5` |  | SNOMED | 3 | Emergency hospital admission for asthma, Emergency asthma admission since las... | 7a5550bd |
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs6` |  | SNOMED | 1 | Number of asthma exacerbations in past year | 0a99b44e |
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs7` |  | SCT Const | 3 | Prednisolone, Prednisolone Sodium Phosphate, Prednisolone Steaglate | 153a3cb0 |
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs8` |  | SCT Const | 5 | Theophylline, Theophylline Hydrate, Theophylline S/R +2 more | 08d2e32b |
| On Asthma(CYP) Register- LTC LCS Priority Group 2 (HR)* | `on_asthma_cyp_reg_pg2_hr_vs9` |  | SCT Const | 3 | Salbutamol, Salbutamol Cr, Terbutaline Sulfate | 5c851fb5 |
