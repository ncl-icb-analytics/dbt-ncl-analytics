{% macro get_ltc_lcs_medication_orders_latest(valueset_ids=none, valueset_friendly_names=none) %}

{%- if valueset_ids is none and valueset_friendly_names is none -%}
    {{ exceptions.raise_compiler_error("get_ltc_lcs_medication_orders_latest requires either valueset_ids or valueset_friendly_names parameter") }}
{%- endif -%}
...
{%- endif %}
qualify row_number() over (partition by pp.person_id, ec.valueset_id order by mo.order_date desc, mo.medication_order_id desc) = 1
