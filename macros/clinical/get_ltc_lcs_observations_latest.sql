{% macro get_ltc_lcs_observations_latest(valuesets, match_paths='both') %}

{#
    Returns the latest observation per (person, valueset) for one or more LTC LCS valuesets
    ...
    to translate EMIS codes to SNOMED. Latest is picked across both paths combined; ties on
    date prefer the 'mapped' row.
...
{%- if match_paths not in allowed_paths -%}
    {{ exceptions.raise_compiler_error("get_ltc_lcs_observations_latest match_paths must be one of: both, mapped, source") }}
{%- endif -%}
