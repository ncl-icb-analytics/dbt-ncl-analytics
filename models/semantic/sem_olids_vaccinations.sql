...
COMMENT = 'OLIDS Vaccinations Semantic View - COVID and flu uptake by person, campaign and risk group; current-state pneumococcal, shingles and RSV vaccination status. COVID and flu are one row per person per campaign per risk group: headcounts must use COUNT(DISTINCT person_id), never COUNT(*) or flag sums.'
AI_SQL_GENERATION 'COVID/FLU GRAIN WARNING: one row per person per campaign per risk group. Eligible people appear once per risk group, so headcounts MUST use COUNT(DISTINCT person_id), never COUNT(*) or SUM of flags. Always filter programme_type and campaign_id before calculating uptake. Uptake rate = vaccinated_patients / eligible_patients within ONE programme_type and campaign_id. Campaigns are COVID Autumn 2024, COVID Spring 2025, COVID Autumn 2025, COVID Spring 2026, Flu 2024-25, and Flu 2025-26. Flu LAIV counts as vaccinated. Pneumococcal, shingles and RSV are current-state single-programme tables with no campaign history. Their cohorts are: pneumococcal aged 65+ or PPV clinical risk group; shingles 70-79 catch-up, turned 65 after September 2023, or immunosuppressed aged 18+; RSV aged 75+, care home resident, or pregnant aged 12-55. Population includes inactive/deceased people: filter is_active = TRUE for coverage. CROSS-VIEW COHORT INTERSECTION: person_id joins to other sem_olids_* views. Put each SEMANTIC_VIEW() call in its own CTE selecting person_id, join CTEs on person_id, then aggregate; person_id never appears in final output. Examples: flu uptake x COPD or diabetes registers in sem_olids_population, or shingles x immunosuppressed cohorts. Apply HAVING COUNT(DISTINCT person_id) > 5 to final output.'
