...
AI_SQL_GENERATION 'CROSS-VIEW COHORT INTERSECTION: person_id is a pseudonymised join key shared by all sem_olids_* views. To intersect cohorts across views (e.g. biomarker trajectory here x medication in sem_olids_prescribing), put each SEMANTIC_VIEW() call in its own CTE selecting person_id plus the needed dimensions/facts, JOIN the CTEs on person_id, then aggregate. person_id must NEVER appear in the final SELECT, GROUP BY, or ORDER BY — results must be aggregate counts/rates with the same small-number suppression as any other aggregate. Always filter to a single observation_type before aggregating or comparing value — values across types are not comparable (different units). For latest-N questions ("last 2 HbA1c"), use ROW_NUMBER() OVER (PARTITION BY person_id, observation_type ORDER BY clinical_effective_date DESC, observation_event_id DESC) and filter the rank — the event id is a deterministic tiebreaker for multiple readings on the same date. For CHANGE / trajectory questions ("whose BMI or waist circumference rose in the last X months"), per person take the latest reading and a baseline reading (the most recent reading on or before DATEADD(month, -X, latest_date)), then difference them; require both to exist. Blood pressure is two types: Systolic BP and Diastolic BP. Always filter to is_active = TRUE unless asked otherwise. RETENTION: full history is retained for currently-registered persons but only ~5 years for left/deceased persons, so this view is NOT capped at 60 months — use it for per-person trajectories and change within the currently-registered cohort. Do NOT compute long-run population cross-sections (e.g. average value by calendar year going back >5 years) as if representative — pre-5-year data is survivor-biased toward people still registered. This view is for over-time analysis; for the single latest value per biomarker use sem_olids_observations.'
AI_QUESTION_CATEGORIZATION 'Use this view for: serial biomarker readings, latest-2 / last-N values, trends in an individual or cohort over time, variability, rate of change, time between readings, and recheck intervals. For the single most recent value per biomarker (current state) use sem_olids_observations. For condition prevalence/demographics use sem_olids_population. For condition incidence/prevalence trends use sem_olids_trends. Questions needing cohorts from TWO domains (e.g. biomarker trajectory x medication exposure) are answerable by joining this view to the other sem_olids_* views on person_id in CTEs, with aggregate-only output.'
