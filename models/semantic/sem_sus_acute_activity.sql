...
AI_SQL_GENERATION 'APC is admission-spell grain, UEC is A&E-attendance grain, and OP is ATTENDED outpatient-appointment grain. Never mix settings in one query. Cost is the SUS listed/national price, NOT actual provider cost; use sem_cost_index for SLAM actual provider cost. REVIEW: coverage window has not yet been validated. CROSS-DATASET COHORT INTERSECTION: create one CTE from sem_olids_population selecting sk_patient_id and cohort filters, one CTE here selecting the setting''s sk_patient_id and event filters, join on sk_patient_id, then aggregate. Never return sk_patient_id in final output; apply HAVING COUNT(DISTINCT sk_patient_id) > 5.'
