...
COMMENT = 'OLIDS LTC LCS Semantic View - case-finding candidates and the risk-stratified LTC LCS register population with Model of Care progress. Do not mix case-finding and risk-summary elements: they are different populations. Pathway-neutral stage flags avoid double counting the same review event.'
AI_SQL_GENERATION 'CASE FINDING is for candidates with programme indicators, not people already diagnosed: the purpose is to identify them. RISK SUMMARY is the risk-stratified LTC LCS register population with Model of Care pathway progress. Never mix cf and rs elements in one query because they are different populations. Indicator numbers 61-66 are programme-specific; hypertension has no indicator 64. Risk summary includes deceased and inactive people, so filter is_active = TRUE unless asked otherwise. The same review event may appear as moc_remote_desktop_review_* on MRS/LRS or moc_mdt_review_* on HRCS/HRS; never sum both, and use the pathway-neutral stage flags exposed here. CROSS-VIEW COHORT INTERSECTION: put each SEMANTIC_VIEW() call in a CTE selecting person_id, join on person_id, then aggregate; never return person_id. Examples: case-finding candidates x appointment DNA, or HRC patients x polypharmacy in sem_olids_population. Apply HAVING COUNT(DISTINCT person_id) > 5 to final output.'
