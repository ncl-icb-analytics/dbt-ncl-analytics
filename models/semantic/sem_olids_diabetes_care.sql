...
AI_SQL_GENERATION 'CROSS-VIEW COHORT INTERSECTION: person_id is a pseudonymised join key shared by all sem_olids_* views. To intersect cohorts across views (e.g. care-process gaps here x DNA in sem_olids_appointments, or targets here x GLP-1/SGLT2 orders in sem_olids_prescribing), put each SEMANTIC_VIEW() call in its own CTE selecting person_id, JOIN the CTEs on person_id, then aggregate; person_id must NEVER appear in the final SELECT/GROUP BY/ORDER BY, and apply small-number suppression to the final output. Always filter is_active = TRUE unless asked otherwise. Completion rates: all_8_count / dm_register_count (and per-process *_done_count over dm_register_count). Targets use LATEST-EVER values with separate recency flags — "recently measured but out of range" is the *_recent_but_out_of_range dimensions. The foot model''s recency window uses month-boundary arithmetic and can differ slightly from cp.foot_check_completed_in_last_12m — prefer the cp flag for care-process reporting and the foot model for risk-level/exemption detail. 12-month windows are relative to the build date, not the QOF year end.'
