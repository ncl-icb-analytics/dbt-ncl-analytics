/*
COVID Vaccination Status Fact Table

This model provides comprehensive vaccination status tracking including:
1. All vaccination activity (administered, declined) with eligibility status
...
-- All vaccination administration records (all campaigns automatically included from intermediate models)
vaccination_administration AS (
    -- COVID Vaccination Administration
    SELECT 
    ...
    FROM {{ ref('int_covid_vaccination_given') }}
),
...
vaccination_declined AS (
    ...
    FROM {{ ref('int_covid_vaccination_declined') }}
),
...
final_status AS (
    SELECT 
        ...
        CASE
            WHEN aep.person_id IS NULL AND avs.status_type = 'VACCINATION_ADMINISTERED' 
            THEN TRUE 
            ELSE FALSE 
