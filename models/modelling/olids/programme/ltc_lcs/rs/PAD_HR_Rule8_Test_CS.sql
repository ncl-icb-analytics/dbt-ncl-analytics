...
--Must have (exclude): Age <80 years
--Then same date: 24hr BP (Refset) in last 12 months
--Then same date: Systolic BP >0, latest
...
--Then same date: Systolic BP >0, latest (check value 1-145)
--(Excludes if passed, includes if failed)
...
on DR.PERSON_ID = BP.PERSON_ID
 ...
 where age.AGE >= 80
and
  BP.DIASTOLIC_VALUE <= 85    
  and
 BP.SYSTOLIC_VALUE <= 145
