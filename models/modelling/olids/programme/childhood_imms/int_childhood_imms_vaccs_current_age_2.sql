...
--HELPER COLUMN to check number of months between DOB and vaccination date to check not > 24 months
        ...
        AND (v1.BORN_SEP_2022_FLAG OR v1.BORN_JUL_2024_FLAG)
) 
...
--HELPER column number of months between second vaccination date and approx first bday. If negative then EVENT is early and not valid
...
--FOR THOSE BORN BEFORE 1st JULY 2024
    --6-IN-1 3X doses anytime before or on second bday. THIRD sixin1 AGE AT EVENT must be less than or equal to 2 and check that event age in mths is <= 24 mths 
    ...
    --hibmenc AGE AT EVENT must be less than or equal to 2 and check that event age in mths is <= 24 mths 
   ,CASE WHEN h.hibmc_first_event_age <= 2 AND h.hibmc_event_age_mths <= 24 THEN 1 ELSE 0  END AS hibmc_comp_by_2
   --MMR Dose 1 on or after their first birthday and before or on second birthday for those born on or after 1st September 2022 OR on before 1st July 2024
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------
--APPLIES TO ALL
    --MenB 3rd dose anytime before or on second bday.THIRD menB AGE AT EVENT must be less than or equal to 2 and check that event age in mths is <= 24 mths
    ,CASE 
