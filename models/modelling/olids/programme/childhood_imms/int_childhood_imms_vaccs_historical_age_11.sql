...
,SIXIN1 AS (
    ...
    LEFT JOIN HIST11YRBASE v2 ON v1.PERSON_ID = v2.PERSON_ID AND v2.VACCINE_ID = '6IN1_2' AND v2.EVENT_TYPE = 'Administration'
    LEFT JOIN HIST11YRBASE v3 ON v1.PERSON_ID = v3.PERSON_ID AND v3.VACCINE_ID = '6IN1_3' AND v3.EVENT_TYPE = 'Administration'
...
--HELPER column number of months between first vaccination and second vaccination. Must be >= 3
 ...
 --HELPER COLUMN to check number of months between DOB and second vaccination date is not >132 months (unlikely)
    ROUND(MONTHS_BETWEEN(v2.EVENT_DATE, v1.BIRTH_DATE_APPROX)) AS mmr_second_event_age_mths,
         ...
         FROM HIST11YRBASE v1
        LEFT JOIN HIST11YRBASE v2 ON v1.PERSON_ID = v2.PERSON_ID AND v2.VACCINE_ID = 'MMR_2' and v2.EVENT_TYPE = 'Administration'
