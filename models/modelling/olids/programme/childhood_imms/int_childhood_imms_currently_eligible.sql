...
CASE 
WHEN VACCINE_ID in ('PCV_1B') AND (BORN_JUL_2024_FLAG OR BORN_JAN_2025_FLAG) THEN TRUE 
WHEN VACCINE_ID in ('MENB_2B') AND (BORN_JUL_2024_FLAG OR BORN_JAN_2025_FLAG) THEN TRUE 
WHEN VACCINE_ID in ('6IN1_4') AND (BORN_JUL_2024_FLAG OR BORN_JAN_2025_FLAG) THEN TRUE 
WHEN VACCINE_ID in ('MMRV_1','MMRV_2') AND BORN_JAN_2025_FLAG THEN TRUE 
WHEN VACCINE_ID in ('MMRV_1B','MMRV_2B') AND BORN_JUL_2024_FLAG THEN TRUE 
WHEN VACCINE_ID in ('MMRV_1C') AND BORN_SEP_2022_FLAG THEN TRUE 
ELSE FALSE END AS NEW_VACCINE_APPLICABLE
...
WHEN AGE_DAYS_APPROX >= ELIGIBLE_AGE_FROM_DAYS
    AND AGE_DAYS_APPROX <= ELIGIBLE_AGE_TO_DAYS THEN TRUE ELSE FALSE 
...
WHEN vaccine_name like 'MMR%' AND NEW_VACCINE_APPLICABLE AND CURRENTLY_ELIGIBLE THEN TRUE
WHEN vaccine_name like 'MMR%' AND NEW_VACCINE_APPLICABLE = FALSE AND CURRENTLY_ELIGIBLE THEN FALSE
--HibMenC no longer offered for those born on or after July 2024. Replaced by 6IN1 Dose 4 at 18 mths
WHEN vaccine_name = 'Hib/MenC' AND (BORN_JUL_2024_FLAG OR BORN_JAN_2025_FLAG ) THEN FALSE
...
WHEN vaccine_id in ('PCV_1','MENB_2') AND NEW_VACCINE_APPLICABLE AND CURRENTLY_ELIGIBLE THEN TRUE
WHEN vaccine_id in ('PCV_1','MENB_2') AND (BORN_JUL_2024_FLAG OR BORN_JAN_2025_FLAG ) THEN FALSE
