...
--Capture the RSV dose 1 vaccination events for this population
  SELECT 
        v.PERSON_ID
         ,v.AGE_65_PLUS
        ,v.AGE_75_PLUS
       ,v.AGE_BAND_5Y
        ...
        ,v.IMDQUINTILE_ORDER
        ,v.practice_code
        ,v.EVENT_DATE AS rsv_dose1_date 
        ,TO_NUMBER(TO_CHAR(v.event_date, 'YYYYMM')) AS rsv_dose1_sort
        ,v.FISCAL_YEAR as rsv_dose1_fiscal
        ,MONTHNAME(v.EVENT_DATE) || '-' || YEAR(v.EVENT_DATE) AS rsv_dose1_label
       ...
       WHERE v.VACCINE_ID = 'RSV_1' AND v.EVENT_TYPE LIKE 'Admin%'
       --select latest RSV event for each person.
