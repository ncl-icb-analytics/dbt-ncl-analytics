...
/*
Monthly eFI2 rules engine — turns the history of eligible eFI2 observations into
per-person, per-month deficit flags.

Port of lds-pipelines stg_efi2__rules. Substitutions vs lds (OLIDS-native):
- lds stg_efi2__cohort_snomed_codes   -> int_efi2_cohort_snomed_codes_history
- Anaemia haemoglobin gate: lds base_phenolab__dev_measurements
  (measurement_haemoglobin_SNOMED) -> int_haemoglobin_all, latest as at month-end.
...
with
    rules_needed as (select * from {{ ref("int_efi2_cohort_snomed_codes_history") }}),
    ...
    haemoglobin_as_of as (
        ...
        left join {{ ref("int_haemoglobin_all") }} hb
    ...
    anaemia as (
        select distinct
            ...
            -- int_haemoglobin_all (standardised to g/L), so the equivalent
            ...
            ac.last_date
        from anaemia_code ac
        left join haemoglobin_as_of hb
            on ac.person_id = hb.person_id
            and ac.end_date = hb.end_date
    ),
    ...
    unioned as (
        ...
        select
            ...
            end_date,
            clinical_effective_date as last_date
        from {{ ref("int_efi2_cohort_snomed_codes_history") }}
...
select distinct person_id, deficit, other_instructions, has_deficit, sub_deficit, end_date, last_date
from unioned
