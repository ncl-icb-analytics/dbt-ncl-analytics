...
/*
Monthly eFI2 chronology — attaches weights to each person's month-end deficit
flags, applies the special-case rules and sums the score.

Port of lds-pipelines stg_efi2__chronology. Substitutions vs lds (OLIDS-native):
- lds stg_efi2__rules                -> int_efi2_rules_history
...
polypharmacy are evaluated) draws from int_efi2_patient_list_history, the full
scored spine, rather than int_efi2_cohort_snomed_codes_history. See the
person_unique CTE for why
...
with
    deficit_weights as (
        select
            ...
            ew.detail_key,
            ew.weight
        from {{ ref("int_efi2_rules_history") }} id
    ...
    -- int_efi2_cohort_snomed_codes_history, so it is not re-applied here.
    --
    ...
    -- spine, NOT int_efi2_cohort_snomed_codes_history. The cohort table is an inner join to
    ...
    person_unique as (
        select distinct person_id, end_date from {{ ref("int_efi2_patient_list_history") }}
    ),
