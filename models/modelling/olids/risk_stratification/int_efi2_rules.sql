...
/*
eFI2 rules engine — turns the raw eFI2-coded observations into per-person deficit
flags (has_deficit / sub_deficit), applying the deficit-specific threshold logic
(blood-pressure counts, alcohol bands, BMI, CKD, cognitive scores, etc.).
...
with
    rules_needed as (select * from {{ ref("int_efi2_cohort_snomed_codes") }}),
    ...
    anaemia as (
        select distinct
            ...
            -- int_haemoglobin_latest (standardised to g/L), so the equivalent
        ...
        left join {{ ref("int_haemoglobin_latest") }} hb
...
select person_id, deficit, other_instructions, has_deficit, sub_deficit, end_date, last_date
from unioned
