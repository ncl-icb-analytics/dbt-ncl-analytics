...
Numerator: Routine attended appointments with days_to_appointment <= 14
...
select
    ...
    SUM(CASE WHEN days_to_appointment <= 14 THEN 1 ELSE 0 END) as routine_within_14d,
    ROUND(
        100.0 * SUM(CASE WHEN days_to_appointment <= 14 THEN 1 ELSE 0 END) / NULLIF(COUNT(*), 0),
        1
