...
-- to the winning file per (provider, FY, month) via
-- stg_mh_slam_latest_submission. Sum-safe; the cumulative Flex/Freeze
-- restatements are excluded. Full submission history is in stg_mh_referrals.
-- Rows with no resolvable period are not included (they cannot be assigned
-- to a slice).
