{{
    config(
        description="Raw layer (Community services dataset). 1:1 passthrough with cleaned column names. \nSource: DATA_LAKE.CSDS.CYP001MPI \ndbt: source(''csds'', ''CYP001MPI'') \nColumns:\n  SK -> sk\n  LOCAL PATIENT IDENTIFIER (EXTENDED) -> local_patient_identifier_extended\n  ORGANISATION IDENTIFIER (LOCAL PATIENT IDENTIFIER) -> organisation_identifier_local_patient_identifier\n  ORGANISATION CODE (LOCAL PATIENT IDENTIFIER) -> organisation_code_local_patient_identifier\n  ORGANISATION IDENTIFIER (RESIDENCE RESPONSIBILITY) -> organisation_identifier_residence_responsibility\n  ORGANISATION CODE (RESIDENCE RESPONSIBILITY) -> organisation_code_residence_responsibility\n  ORGANISATION IDENTIFIER (EDUCATIONAL ESTABLISHMENT) -> organisation_identifier_educational_establishment\n  ORGANISATION CODE (EDUCATIONAL ESTABLISHMENT) -> organisation_code_educational_establishment\n  NHS NUMBER STATUS INDICATOR CODE -> nhs_number_status_indicator_code\n  DateOfBirth Pseudo -> date_of_birth_pseudo\n  Postcode Pseudo -> postcode_pseudo\n  PERSON STATED GENDER CODE -> person_stated_gender_code\n  ETHNIC CATEGORY -> ethnic_category\n  ETHNIC CATEGORY 2021 -> ethnic_category_2021\n  LANGUAGE CODE (PREFERRED) -> language_code_preferred\n  PERSON RELATIONSHIP (MAIN CARER) -> person_relationship_main_carer\n  HEALTH VISITOR FIRST ANTENATAL VISIT DATE -> health_visitor_first_antenatal_visit_date\n  LOOKED AFTER CHILD INDICATOR -> looked_after_child_indicator\n  SAFEGUARDING VULNERABILITY FACTORS INDICATOR -> safeguarding_vulnerability_factors_indicator\n  CONSTANT SUPERVISION AND CARE REQUIRED DUE TO DISABILITY INDICATOR -> constant_supervision_and_care_required_due_to_disability_indicator\n  EDUCATIONAL ASSESSMENT OUTCOME -> educational_assessment_outcome\n  PREFERRED DEATH LOCATION DISCUSSED INDICATOR -> preferred_death_location_discussed_indicator\n  PERSON AT RISK OF UNEXPECTED DEATH INDICATOR -> person_at_risk_of_unexpected_death_indicator\n  DEATH LOCATION TYPE CODE (PREFERRED) -> death_location_type_code_preferred\n  PERSON DEATH DATE -> person_death_date\n  DEATH LOCATION TYPE CODE (ACTUAL) -> death_location_type_code_actual\n  DEATH NOT AT PREFERRED LOCATION REASON -> death_not_at_preferred_location_reason\n  NHS NUMBER STATUS INDICATOR CODE (MOTHER) -> nhs_number_status_indicator_code_mother\n  EFFECTIVE FROM -> effective_from\n  RECORD NUMBER -> record_number\n  CYP001 UNIQUE ID -> cyp001_unique_id\n  ORGANISATION IDENTIFIER (CODE OF PROVIDER) -> organisation_identifier_code_of_provider\n  ORGANISATION CODE (PROVIDER) -> organisation_code_provider\n  PERSON ID -> person_id\n  UNIQUE CSDS ID (PATIENT) -> unique_csds_id_patient\n  UNIQUE SUBMISSION ID -> unique_submission_id\n  BSP UNIQUE ID -> bsp_unique_id\n  VALID NHS NUMBER FLAG -> valid_nhs_number_flag\n  VALID POSTCODE FLAG -> valid_postcode_flag\n  POSTCODE DISTRICT -> postcode_district\n  Postcode_Default Pseudo -> postcode_default_pseudo\n  LOWER SUPER OUTPUT AREA (RESIDENCE) -> lower_super_output_area_residence\n  LOWER SUPER OUTPUT AREA (RESIDENCE) 2011 -> lower_super_output_area_residence_2011\n  CENSUS YEAR -> census_year\n  COUNTY -> county\n  LOCAL AUTHORITY DISTRICT/UNITARY AUTHORITY -> local_authority_district_unitary_authority\n  ELECTORAL WARD OF USUAL ADDRESS -> electoral_ward_of_usual_address\n  AGE OF PATIENT AT REPORTING PERIOD START (DAYS) -> age_of_patient_at_reporting_period_start_days\n  AGE OF PATIENT AT REPORTING PERIOD END (DAYS) -> age_of_patient_at_reporting_period_end_days\n  AGE AT DEATH (DAYS) -> age_at_death_days\n  ORGANISATION IDENTIFIER (CCG OF RESIDENCE) -> organisation_identifier_ccg_of_residence\n  ORGANISATION CODE (CCG OF RESIDENCE) -> organisation_code_ccg_of_residence\n  AGE OF PATIENT AT REPORTING PERIOD START (YEARS) -> age_of_patient_at_reporting_period_start_years\n  IC_AGE_OF_PATIENT_AT_RP_START -> ic_age_of_patient_at_rp_start\n  AGE OF PATIENT AT REPORTING PERIOD END (YEARS) -> age_of_patient_at_reporting_period_end_years\n  IC_AGE_OF_PATIENT_AT_RP_END -> ic_age_of_patient_at_rp_end\n  AGE AT DEATH (YEARS) -> age_at_death_years\n  IC_AGE_AT_DEATH -> ic_age_at_death\n  RECORD START DATE -> record_start_date\n  RECORD END DATE -> record_end_date\n  UNIQUE MONTH ID -> unique_month_id\n  AGE GROUP (REPORTING PERIOD START DATE) -> age_group_reporting_period_start_date\n  AGE GROUP (REPORTING PERIOD END DATE) -> age_group_reporting_period_end_date\n  AGE GROUP (DEATH DATE) -> age_group_death_date\n  AGE BAND (REPORTING PERIOD START DATE) -> age_band_reporting_period_start_date\n  AGE BAND (REPORTING PERIOD END DATE) -> age_band_reporting_period_end_date\n  AGE BAND (DEATH DATE) -> age_band_death_date\n  ORGANISATION IDENTIFIER (SUB ICB LOCATION OF RESIDENCE) -> organisation_identifier_sub_icb_location_of_residence\n  ORGANISATION IDENTIFIER (ICB OF RESIDENCE) -> organisation_identifier_icb_of_residence\n  dmicImportLogId -> dmic_import_log_id\n  dmicEthnicCategoryGroup -> dmic_ethnic_category_group\n  dmicSystemId -> dmic_system_id\n  dmicCCGCode -> dmic_ccg_code\n  dmicCCG -> dmic_ccg\n  Unique_LocalPatientId -> unique_local_patient_id\n  UniqueCYPHS_ID_Patient -> unique_cyphs_id_patient\n  dmIcbResidenceSubmitted -> dm_icb_residence_submitted\n  dmSubIcbResidenceSubmitted -> dm_sub_icb_residence_submitted\n  dmCommissionerDerivationReason -> dm_commissioner_derivation_reason\n  FILE TYPE -> file_type\n  REPORTING PERIOD START DATE -> reporting_period_start_date\n  REPORTING PERIOD END DATE -> reporting_period_end_date"
    )
}}
select
    "SK" as sk,
