{{
    config(
        description="Raw layer: NHS communication-method language extensions and their UKHFD history. 1:1 passthrough with cleaned column names."
    )
}}
select
...
from {{ source('ukhfd_data_dictionary', 'language_code') }}
