{{
    config(
        description="Raw layer: Mental health admitted patient classification type definitions and their UKHFD history. 1:1 passthrough with cleaned column names."
    )
}}
select
...
from {{ source('ukhfd_data_dictionary', 'mental_health_admitted_patient_classification_type') }}
