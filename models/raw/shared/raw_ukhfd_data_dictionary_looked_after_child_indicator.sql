{{
    config(
        description="Raw layer: Looked-after-child indicator definitions and their UKHFD history. 1:1 passthrough with cleaned column names."
    )
}}
select
...
from {{ source('ukhfd_data_dictionary', 'looked_after_child_indicator') }}
